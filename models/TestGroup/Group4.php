...
{   
    ...
    function render()
    {
        ...
        foreach($report as $key => $re)
        {
            ...
            $rows .= '<td>'.number_format($re['skor']['TOTAL']/160*100,2).'</td>';
            $rows .= '<td>'.self::category2($re['skor']['TOTAL']/160*100).'</td>';
            $rows .= '<td>'.$subskor.'</td>';
            $rows .= '<td>'.number_format($subskor_pre,2).'</td>';
            ...
            $rows .= '<td>'.number_format($holland_skor_pre,2).'</td>';
            $rows .= '<td>'.self::category2($holland_skor_pre).'</td>';
            $rows .= '<td>'.$imj_skor.'</td>';
            $rows .= '<td>'.number_format($imj_skor_pre,2).'</td>';
            ...
            $rows .= '<td>'.number_format($holland_skor_pre,2).'</td>';
            $rows .= '<td>'.number_format($imj_skor_pre,2).'</td>';
            $rows .= '<td>'.number_format(($imj_skor_pre*0.5)+($holland_skor_pre*0.5),2).'</td>';
            $rows .= '<td>'.number_format($last_pre,2).'</td>';
